...
class RunSparkHandler(tornado.web.RequestHandler):
	...
	def process_res_extractmzs(self, result):
		...
		self.insert_job_result_stats( [ self.formula_id ], [ len(res_array) ], [ { "entropies" : entropies } ] )
	...
	def process_res_fulldataset(self, result, offset=0):
		...
		self.insert_job_result_stats(
			...
			[ { "entropies" : entropies[i] } for i in xrange(len(res_dicts)) ]
		)
	...
	def post(self, query_id):
		...
		if query_id == "extractmzs":
			...
			tol = 0.01
			self.formulas = self.db.query("SELECT formula_id as id,peaks FROM mz_peaks LIMIT 100")
